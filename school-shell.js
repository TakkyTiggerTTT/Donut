...
let currentSettings = { wipEnabled: false, wipMessage: "Donut School is being updated. Please check again later." };
...
function enforceAccess() {
  ...
  if (currentSettings.wipEnabled) {
    if (isOwnerPage && !authResolved) return;
  ...
  if (isOwnerPage && authResolved && currentProfile?.role !== "owner") {
    location.replace(auth.currentUser ? "./dashboard.html" : "./index.html");
  }
}
...
function navItems(role) {
  ...
  const items = [
    ...
    ["owner.html", "👑", "Owner", role === "owner"]
  ];

  return items.filter((item) => item[3]).map(([file, icon, label]) => `
...
onAuthStateChanged(auth, async (user) => {
  ...
  if (user) {
    try {
      ...
      if (profileSnapshot.exists()) currentProfile = { id: user.uid, ...profileSnapshot.data() };
    } catch (error) {
